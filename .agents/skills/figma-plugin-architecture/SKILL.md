...
description: Informações sobre como os arquivos se relacionam e fluxo de comunicação no DSI - Documentation (code.ts vs ui.html).
---

# Figma Plugin Architecture

## Quando Usar
Esta skill detalha as camadas de dados e comunicação do plugin. Útil quando você precisar refatorar o ciclo de vida (ex: mudar onde dados do usuário são salvos via clientStorage), adicionar novas abas no `ui.html`, ou modificar como a lógica central (`code.ts`) gerencia erros vindos do front-end.

## Princípios
O plugin DSI - Documentation segue o modelo padrão de plugins do Figma, onde a lógica principal se encontra na Main Thread (`code.ts`) e interage com o Canvas, enquanto a interface do usuário reside no UI (`ui.html`). O uso de memória e rede do UI (`fetch` pro Gemini, por ex) é evitado transferindo a chamada `fetch` diretamente para o ambiente nativo, que agora suporta acesso à rede no Manifest v3, permitindo rodar tudo centralizado no `code.ts`.
